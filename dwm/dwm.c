...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation,  NetSystemTrayVisual,
       ...
       NetSystemTrayOrientationHorz, NetWMWindowTypeDialog, NetClientList, NetWMWindowsOpacity, NetWMCheck, NetLast }; /* EWMH atoms */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
/* 2px right padding */
		else if (ev->x > (selmon->ww - (int)TEXTW(stext) - getsystraywidth() + lrpad - 2 * sp) - 2) {
			x = selmon->ww - TEXTW(stext) - getsystraywidth() + lrpad - 2 - 2 * sp;
 			click = ClkStatusText;
			char *text = rawstext;
			int i = -1;
			char ch;
...
void
runAutostart(void) {
	system("/usr/local/share/dwm/autostart.sh");
}
...
updatesystray(int updatebar)
{
	...
	unsigned int w = 1, xpad = sp, ypad = vp;
