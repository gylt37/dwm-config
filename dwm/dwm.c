...
/* 2px right padding */
			...
			x = selmon->ww - TEXTW(stext)- getsystraywidth()  + lrpad - 2;
 			click = ClkStatusText;
			char *text = rawstext;
			int i = -1;
			char ch;
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x, m->by+vp, w-sp, bh, 0, 0, scheme[SchemeSel][ColBg].pixel);
	...
	x -= w;
	XMoveResizeWindow(dpy, systray->win, x, m->by+vp, w-sp, bh);
	wc.x = x; wc.y = m->by+vp; wc.width = w-sp; wc.height = bh;
